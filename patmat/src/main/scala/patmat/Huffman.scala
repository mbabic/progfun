...
object Huffman {
  ...
  /**
   * Returns the weight of the given instance of CodeTree.
   */
  ...
  /**
   * Returns the chars of the given instance of CodeTree.
   */
   ...
   */
  def until(singletonOp: List[CodeTree] => Boolean,
    combinationOp: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (singletonOp(trees)) trees
    else until(singletonOp, combinationOp)(combinationOp(trees))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def acc(subTree: CodeTree, subBits: List[Bit], chars: List[Char]): List[Char] = {
      subBits match {
        case List() =>
          subTree match {
            case Leaf(c, _) => chars :+ c
        ...
        case x :: xs =>
          subTree match {
            case Leaf(c, _) =>
              acc(tree, subBits, chars :+ c)
            case Fork(left, right, _, _) =>
              if (x == 0) acc(left, xs, chars)
              else acc(right, xs, chars)
          }
      }
    }
    acc(tree, bits, List())
